...
Acuity is an on-chain publishing platform being developed on Polkadot. In its previous incarnation it was called MIX and built on an EVM chain with all of the on-chain logic implemented as Solidity smart contracts. These have been archived into a [monorepo](https://github.com/acuity-network/acuity-contracts/). Content was published to [IPFS](https://ipfs.tech/) and the content hashes stored in contract state.
...
A dapp will be created to demonstrate how content_pallet can be used be build a fully decentralized social media platform.

Additionally, a screencast will be recorded to demonstrate the dapp.
...
Currently Polkadot governance portals such as Polkassembly and Subsquare have centralized referendum discussion functionality beyond what is provided on-chain. This leads to issues such as the need for comment synchronization between these two platforms.

This functionality really needs to be on-chain. This is what pallet_content can provide.

## Benefit to Kusama Ecosystem

How will pallet_content benefit Kusama?

Every chain needs to publish content of one kind or another. For example, a prediction market chain needs to be able to publish what it is that is being predicted.

Once pallet_content exists it will be possible to create dedicated "content parachains" that use Kusama as their relay chain. For example:

* Feeds parachain - replace X, YouTube, RSS
* Topics parachain - replace Reddit
* Forums parachain - replace Discourse 
* Wiki parachain - replace Wikipedia
